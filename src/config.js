...
const generateConfig = async (templateConfigurations) => {
  ...
  if (templateConfigurations.hasOwnProperty("templateVariables")) {
    try {
      ...
      console.error("Error parsing templateVariables:", error);
  ...
  return new Promise(async (resolveMain) => {
        ...
        fs.readdir(
          ...
          (err, files) => {
            if (err) {
              ...
              resolve({ files: [], folder }); // Resolve with an empty array to prevent crash
            } else {
              resolve({
                files,
                folder,
    ...
    try {
        ...
        files.forEach(async (file) => {
          allPromises.push(
            readAndMakeTemplate(file)
              ...
              .catch((err) => {
                console.error("Error processing template for file:", file, err);
              }),
          );
        });
        try {
      ...
      console.error("Error during initial folder read:", error);
      resolveMain([]); // Resolve with an empty array in case of initial error.
    }
  });
};
...
const applyConfiguration = async (yamlString) => {
  try {
    ...
    return []; // return empty array so the process doesn't break
  }
};
...
const applyAppTemplate = async (templateConfig) => {
  try {
    ...
    if (processedConfigs.length > 0) {
      ...
      if (namespaceConfig.length > 0) {
        try {
          ...
          console.error("Error applying namespace configuration:", error);
        }
      }
      ...
      for (const configData of otherConfig) {
        try {
    ...
    console.error("Error in applyAppTemplate:", error);
  }
};
